...
def sshfp(filename):
    with subprocess.Popen(["ssh-keygen", "-l", "-v", "-f", filename],
    ...
    visual_key[7] += "   SSHFP = " + __sshfp_type(visual_key) + " 1 " + __dns_format(hashlib.sha1(key_data)) # noqa
    visual_key[9] += "   SSHFP = " + __sshfp_type(visual_key) + " 2 " + __dns_format(hashlib.sha256(key_data)) # noqa
    print("\n".join(visual_key))
